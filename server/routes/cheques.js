...
router.get('/', (req, res) => {
  ...
  if (status) { query += ` AND cheques.status = ?`; params.push(status); }
  if (month && year) {
...
router.post('/', (req, res) => {
  ...
  db.run(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `, [cheque_number, firm_name, customer_id, bank_name, amount,
    received_date || new Date().toISOString().split('T')[0], order_id, notes],
  function(err) {
    if (err) return res.status(500).json({ error: err.message });
