...
const { getCustomerDuesList, getTotalOutstanding } = require('../utils/customerDues');
          ...
          `, [m, year, m, year], (err, upiIncome) => {
            ...
            `, [m, year], (err, expensesByCategory) => {
              ...
              `, [m, year], (err, totalExpenses) => {
                ...
                // 8. Dues — shared customerDues module (Dashboard jaisa hi)
                getCustomerDuesList(db, (err, dues) => {
                  if (err) return res.status(500).json({ error: err.message });

                  // 9. Total outstanding — same shared module as Dashboard
                  getTotalOutstanding(db, (err, totalDues) => {
                    if (err) return res.status(500).json({ error: err.message });
